...
(* Parallel substitution - moved up *)
...
let rec subst_many m t =
  ...
  | Pi (x, a, b) ->
      ...
      Pi (x, subst_many m a, subst_many m' b)
  | Lam (x, d, b) ->
      let m' = List.filter (fun (y, _) -> y <> x) m in
      Lam (x, subst_many m d, subst_many m' b)
  ...
  | Elim (d, p, cases, t') -> 
      Elim (d, subst_many m p, List.map (subst_many m) cases, subst_many m t')
  | _ -> t
...
let rec equal env ctx t1 t2 =
  ...
  | App (f1, a1), App (f2, a2) ->
      equal env ctx f1 f2 && (f1 == f2 || equal env ctx a1 a2)
  | Pi (x, a1, b1), Pi (y, a2, b2) ->
      equal env ctx a1 a2 && 
      (a1 == a2 || equal env (add_var ctx x a1) b1 (subst y (Var x) b2))
  | Lam (x, d1, b1), Lam (y, d2, b2) ->
      equal env ctx d1 d2 && 
      (d1 == d2 || equal env (add_var ctx x d1) b1 (subst y (Var x) b2))
  | Inductive d1, Inductive d2 ->
      d1.name = d2.name && d1.level = d2.level &&
      List.for_all2 (fun (n1, t1) (n2, t2) -> n1 = n2 && equal env ctx t1 t2) 
        d1.params d2.params
  ...
  | Elim (d1, p1, cases1, t1), Elim (d2, p2, cases2, t2) ->
      ...
      List.for_all2 (equal env ctx) cases1 cases2 && equal env ctx t1 t2
  | _ -> false
...
let apply_inductive d args =
  ...
  let m = List.combine (List.map fst d.params) args in
  Inductive { d with constrs = List.map (fun (j, ty) -> (j, subst_many m ty)) d.constrs }
...
let rec reduce env ctx t =
  ...
  | App (f, arg) ->
      ...
      if equal env ctx f f' then App (f, reduce env ctx arg)
      else App (f', arg)
...
(* Type inference with memoization *)
let rec infer env ctx t =
  ...
  | _ ->
      let result = match t with
        ...
        | Universe i -> 
            Printf.printf "Inferring Universe %d: Type%d\n" i (i + 1);
            Universe (i + 1)
        | Pi (x, a, b) ->
            Printf.printf "Inferring Pi %s\n" x;
            ...
            result
        | Lam (x, domain, body) ->
            Printf.printf "Inferring Lam %s: domain = " x; print_term domain; print_endline "";
...
and check_elim env ctx d p cases t' =
  Printf.printf "Checking Elim for %s\n" d.name;
  ...
  Printf.printf "Checking motive against: "; print_term motive_ty_ty; print_endline "";
  let p_ty = infer env ctx p in
  Printf.printf "Inferred motive type: "; print_term p_ty; print_endline "";
  ...
  List.iteri (fun j case ->
    ...
    Printf.printf "Checking case %d against: " j; print_term expected_ty; print_endline "";
